...
import { AlertCircle, ArrowLeft, Calendar, CheckCircle2, Clock, ClipboardList, Flame, RefreshCw, Trophy, Users } from 'lucide-react';
import { Link, useRouter } from '@/router';
import { CountdownTimer, DifficultyBadge, StatusBadge, subjectGradient, TypeBadge } from '@/components/ContestCard';
        ...
        <div className="container-page relative pb-14 pt-8">
          ...
          <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_320px]">
            ...
            <aside className="rounded-3xl bg-slate-950/35 p-6 ring-1 ring-white/20 backdrop-blur-sm">
              ...
              {contest.status === 'Live' && <div className="mb-5 flex items-center gap-3 rounded-2xl bg-success-500/20 p-4 ring-1 ring-success-400/30"><Flame className="h-5 w-5 text-success-300" /><div><p className="text-sm font-bold">Contest is live</p><p className="text-xs text-white/70">Saved answers are judged on the server.</p></div></div>}
              {contest.status === 'Finished' && <div className="mb-5 rounded-2xl bg-white/10 p-4 text-center"><Trophy className="mx-auto h-6 w-6 text-sun-300" /><p className="mt-2 text-sm font-bold">Contest finished</p><p className="text-xs text-white/65">{contest.isFinalized ? 'Final results are available below.' : 'Natijalar judge yoki admin yakunlagach ochiladi.'}</p></div>}
              {actionError && <p role="alert" className="mb-3 rounded-xl bg-error-500/20 p-3 text-xs text-error-100">{actionError}</p>}
              {contest.status === 'Live' && contest.registered && <Link to={`/contests/${contest.slug}/quiz`} className="btn w-full bg-white py-3 text-indigo-700 hover:bg-indigo-50"><ClipboardList className="h-4 w-4" />Contestga kirish</Link>}
