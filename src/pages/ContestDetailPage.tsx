        ...
        <div className="container-page relative pb-14 pt-8">
          ...
          <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_320px]">
            ...
            <aside className="rounded-3xl bg-slate-950/35 p-6 ring-1 ring-white/20 backdrop-blur-sm">
              ...
              {contest.status === 'Live' && <div className="mb-5 flex items-center gap-3 rounded-2xl bg-success-500/20 p-4 ring-1 ring-success-400/30">{contest.mode === 'Gym' ? <Sparkles className="h-5 w-5 text-cyan-200" /> : <Flame className="h-5 w-5 text-success-300" />}<div><p className="text-sm font-bold">{contest.mode === 'Gym' ? 'Gym mashg‘uloti ochiq' : 'Contest is live'}</p><p className="text-xs text-white/70">{contest.mode === 'Gym' ? 'Gym unrated: masalalarni yeching va xohlaganingizcha mashq qiling.' : contest.subjectSlug === 'programming' ? 'Problem set ro‘yxatdan o‘tgan ishtirokchilar uchun ochiq.' : isEnglishExam ? 'Listening, Reading va Writing partlarini yakunlang.' : 'Saved answers are judged on the server.'}</p></div></div>}
