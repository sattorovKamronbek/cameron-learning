...
function ReadingPart({ part, questions, answers, textAnswers, gapFillResponses, matchingConfig, matchingResponses, annotation, annotationsLoading, annotationSaving, cefrExam, locked, savingKey, onAnswer, onClearAnswer, onTextSave, onGapFillSave, onMatchingSave, onSaveAnnotation }: { part: ExamPart; questions: ContestWorkspace['questions']; answers: Record<string, number>; textAnswers: Record<string, string>; gapFillResponses: Record<string, GapFillResponse>; matchingConfig: MatchingWorkspaceConfig | undefined; matchingResponses: Record<string, MatchingResponse>; annotation: ReadingAnnotation | undefined; annotationsLoading: boolean; annotationSaving: boolean; cefrExam: boolean; locked: boolean; savingKey: string | null; onAnswer: (questionId: string, option: number) => void; onClearAnswer: (questionId: string) => void; onTextSave: (questionId: string, value: string) => void; onGapFillSave: (part: ExamPart, blankNumber: number, answer: string) => void; onMatchingSave: (part: ExamPart, speakerNumber: number, optionPosition: number) => void; onSaveAnnotation: (part: ExamPart, note: string, highlights: ReadingHighlight[]) => void }) {
  const gapFill = gapFillBlankNumbers(part.content).length > 0;
  const matching = Boolean(matchingConfig);
  const miniTextCompletion = false;
  ...
  const questionPanel = ieltsPassageTwoStructured ? <IeltsReadingPassageTwoStructuredQuestions questions={questions} answers={answers} textAnswers={textAnswers} locked={locked} savingKey={savingKey} onAnswer={onAnswer} onClearAnswer={onClearAnswer} onTextSave={onTextSave} /> : ieltsPassageThreeStructured ? <IeltsReadingPassageThreeStructuredQuestions questions={questions} answers={answers} textAnswers={textAnswers} locked={locked} savingKey={savingKey} onAnswer={onAnswer} onClearAnswer={onClearAnswer} onTextSave={onTextSave} /> : matching ? <SpeakerMatchingListening part={part} config={matchingConfig} responses={matchingResponses} locked={locked} savingKey={savingKey} mapMode={Boolean(part.imageUrl)} readingMode={matchingMode} showPartContent={false} onSave={onMatchingSave} /> : miniTextCompletion ? <CefrReadingPartFiveMiniTexts questions={questions} answers={answers} textAnswers={textAnswers} locked={locked} savingKey={savingKey} onAnswer={onAnswer} onTextSave={onTextSave} /> : objectiveQuestions;
