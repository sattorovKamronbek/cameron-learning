...
BEGIN
  ...
  IF v_part.position = 4 THEN
    IF NEW.position NOT BETWEEN 21 AND 29 THEN RAISE EXCEPTION 'CEFR Reading Part 4 question numbers must be between 21 and 29'; END IF;
    IF NEW.correct_option IS NULL THEN RAISE EXCEPTION 'Every CEFR Reading Part 4 question needs an answer key'; END IF;
    IF NEW.position BETWEEN 21 AND 24 AND jsonb_array_length(NEW.options) <> 4 THEN RAISE EXCEPTION 'CEFR Reading Part 4 questions 21 through 24 must use four A/B/C/D options'; END IF;
    IF NEW.position BETWEEN 25 AND 29 AND NEW.options <> jsonb_build_array('True', 'False', 'Not Given') THEN RAISE EXCEPTION 'CEFR Reading Part 4 questions 25 through 29 must use True, False, Not Given'; END IF;
  ELSIF v_part.position = 5 THEN
    IF NEW.position NOT BETWEEN 34 AND 35 THEN RAISE EXCEPTION 'CEFR Reading Part 5 question numbers must be 34 or 35'; END IF;
    IF NEW.correct_option IS NULL OR jsonb_array_length(NEW.options) <> 4 THEN RAISE EXCEPTION 'CEFR Reading Part 5 questions 34 and 35 must use four A/B/C/D options and an answer key'; END IF;
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  FOR v_part IN SELECT * FROM public.contest_exam_parts WHERE contest_id = NEW.id AND section = 'reading' LOOP
    v_expected := CASE v_part.position WHEN 1 THEN ARRAY[1,2,3,4,5,6]::integer[] WHEN 2 THEN ARRAY[7,8,9,10,11,12,13,14]::integer[] WHEN 3 THEN ARRAY[15,16,17,18,19,20]::integer[] WHEN 4 THEN ARRAY[21,22,23,24,25,26,27,28,29]::integer[] ELSE ARRAY[34,35]::integer[] END;
    IF v_part.position = 1 THEN
      IF (SELECT array_agg(DISTINCT (marker.values)[1]::integer ORDER BY (marker.values)[1]::integer) FROM regexp_matches(v_part.content, '\{\{([1-9][0-9]*)\}\}', 'g') marker(values)) IS DISTINCT FROM v_expected
        OR (SELECT count(*) FROM public.contest_gap_fill_answer_keys key WHERE key.exam_part_id = v_part.id AND key.blank_number = ANY(v_expected)) <> 6 THEN
        RAISE EXCEPTION 'CEFR Reading Part 1 needs exactly {{1}} through {{6}} and every answer key';
      END IF;
    ELSIF v_part.position IN (2, 3) THEN
      ...
      IF v_count <> array_length(v_expected, 1) OR v_key_count <> array_length(v_expected, 1) OR NOT EXISTS (SELECT 1 FROM public.contest_matching_options WHERE exam_part_id = v_part.id) OR (v_part.position = 3 AND (SELECT count(*) FROM public.contest_matching_options WHERE exam_part_id = v_part.id) <> 8) THEN RAISE EXCEPTION 'CEFR Reading Part % needs its complete matching answer bank and keys', v_part.position; END IF;
    ELSIF v_part.position = 5 THEN
      IF (SELECT array_agg(DISTINCT (marker.values)[1]::integer ORDER BY (marker.values)[1]::integer) FROM regexp_matches(v_part.content, '\{\{([1-9][0-9]*)\}\}', 'g') marker(values)) IS DISTINCT FROM ARRAY[30,31,32,33]::integer[]
        OR (SELECT count(*) FROM public.contest_gap_fill_answer_keys key WHERE key.exam_part_id = v_part.id AND key.blank_number BETWEEN 30 AND 33) <> 4 THEN
        RAISE EXCEPTION 'CEFR Reading Part 5 needs exactly {{30}} through {{33}} and every gap-fill answer key';
      END IF;
      ...
      IF v_count <> 2 OR v_distinct <> 2 OR v_key_count <> 2 THEN RAISE EXCEPTION 'CEFR Reading Part 5 needs A/B/C/D questions 34 and 35 with answer keys'; END IF;
    ELSE
      SELECT count(*)::integer, count(DISTINCT position)::integer, count(*) FILTER (WHERE correct_option IS NOT NULL)::integer INTO v_count, v_distinct, v_key_count FROM public.contest_questions WHERE contest_id = NEW.id AND exam_part_id = v_part.id AND position = ANY(v_expected);
