-- A problem draft may be saved before its tests are entered, generated or imported.
...
BEGIN
  ...
  IF char_length(trim(coalesce(p_title, ''))) < 3
    ...
    OR jsonb_typeof(coalesce(p_test_cases, 'null'::jsonb)) <> 'array' THEN
  ...
  PERFORM public.log_audit_action(
    ...
    jsonb_build_object('publication_scope', p_publication_scope, 'test_case_count', jsonb_array_length(p_test_cases))
  );
  RETURN v_problem_id;
END;
$$;
